...
import { Download, Save, RefreshCw, AlertCircle, CheckCircle2 } from 'lucide-react';

// -------------------------------------------------------------------------
// ⭐️ URL ของ Web App
// -------------------------------------------------------------------------
const DEFAULT_API_URL = 'https://script.google.com/macros/s/AKfycbyvSN9SEuk86NNDmuX3FqG_eveoz98pSKnehQYcFoXZfqI0-YbGftQ4tWaJQkcC4weO2A/exec';

// --- โครงสร้างข้อมูลเริ่มต้นสำหรับการทดสอบ ---
const initialData = [
  { id: 1, timestamp: '26/03/2569 10:00:00', line: 'Line1', partCode: 'P001', partName: 'Gear Box', quantity: 50, unit: 'EA', amount: 120, totalAmount: 6000 },
  { id: 2, timestamp: '26/03/2569 10:15:00', line: 'Line2', partCode: 'P002', partName: 'Motor Shaft', quantity: 200, unit: 'G', amount: 2.5, totalAmount: 500 },
];
...
const getFlexibleValue = (obj, searchKey) => {
  ...
  const foundKey = Object.keys(obj).find(k => k.toLowerCase().replace(/[^a-z0-9]/g, '') === normalizedSearch);
...
export default function App() {
    ...
    try {
      ...
      if (Array.isArray(result) && result.length > 0) {
        ...
        const formattedData = result.map((item, index) => {
          ...
          const amt = parseFloat(getFlexibleValue(item, 'amountthb') || getFlexibleValue(item, 'amount')) || 0;
          const fetchedTotal = parseFloat(getFlexibleValue(item, 'totalamountthb') || getFlexibleValue(item, 'totalamount')) || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!apiUrl) {
      ...
      resetForm();
      showStatus('success', 'บันทึกข้อมูลจำลองสำเร็จ (ยังไม่ได้เชื่อม Google Sheets)');
      return;
    }
  ...
  const resetForm = () => {
    setFormData({ line: 'Line1', partCode: '', partName: '', quantity: '', unit: 'EA', amount: '', totalAmount: 0 });
  };
  ...
  const exportCSV = () => {
    const headers = ['Timestamp', 'Line', 'Part Code', 'Part Name', 'Quantity', 'Unit', 'Amount (THB)', 'Total Amount (THB)'];
    const csvRows = [];
    csvRows.push(headers.join(','));

    filteredData.forEach(row => {
      const values = [
        row.timestamp, row.line, row.partCode, row.partName, row.quantity, row.unit, row.amount, row.totalAmount
      ].map(val => `"${val}"`);
  ...
  const chartDataByLine = useMemo(() => {
    const grouped = {};
    ['Line1', 'Line2', 'Line3', 'Line4', 'Line5'].forEach(l => grouped[l] = []);
    
    filteredData.forEach(item => {
      if (!grouped[item.line]) grouped[item.line] = [];
      const existing = grouped[item.line].find(x => x.partCode === item.partCode);
      if (existing) {
        existing.quantity += item.quantity;
        existing.totalAmount += item.totalAmount;
      } else {
        grouped[item.line].push({
          name: item.partCode, partName: item.partName, quantity: item.quantity, totalAmount: item.totalAmount
        });
      }
    });
    return grouped;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500">ระบบบันทึกและแสดงผลข้อมูลการผลิต</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <div className="flex items-center gap-2 mb-4 border-b pb-2">
              ...
              <h2 className="text-lg font-bold text-slate-800">ฟอร์มบันทึกข้อมูล</h2>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Part Code</label>
                  <input type="text" name="partCode" value={formData.partCode} onChange={handleInputChange} required
                    className="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="e.g. P001" />
                </div>
                <div>
                  ...
                  <input type="text" name="partName" value={formData.partName} onChange={handleInputChange}
                    className="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="e.g. Gear" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" step="any" name="quantity" value={formData.quantity} onChange={handleInputChange} required
                    className="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="0" />
                </div>
                <div>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Amount (THB) / Unit</label>
                <input type="number" step="any" name="amount" value={formData.amount} onChange={handleInputChange} required
                  className="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="0.00" />
              </div>

              <div className="bg-slate-100 p-3 rounded-lg border border-slate-200">
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Total Amount (Auto Calculated)</label>
                <div className="text-2xl font-bold text-indigo-700">
                  ฿ {formData.totalAmount.toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </div>
              </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-full flex flex-col">
              <div className="flex items-center gap-2 mb-4 border-b pb-2">
                ...
                <h2 className="text-lg font-bold text-slate-800">Monitor (แสดงผลแยกตาม Line)</h2>
              </div>
              ...
              <div className="flex-1 overflow-y-auto pr-2" style={{ maxHeight: '600px' }}>
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                  {['Line1', 'Line2', 'Line3', 'Line4', 'Line5'].map((line) => {
                    ...
                    return (
                      <div key={line} className="border border-slate-100 rounded-xl p-4 bg-slate-50 min-w-0">
                        <h3 className="font-bold text-slate-700 mb-4 border-l-4 border-emerald-500 pl-2">{line}</h3>
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={lineData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                              ...
                              <Tooltip 
                                ...
                                formatter={(value, name) => [value.toLocaleString('th-TH'), name === 'quantity' ? 'Quantity' : 'Total Amount (THB)']}
                              />
                              <Legend wrapperStyle={{ paddingTop: '10px' }} />
                              <Bar yAxisId="left" dataKey="quantity" name="Quantity" fill="#3b82f6" radius={[4, 4, 0, 0]} maxBarSize={50} />
                              <Line yAxisId="right" type="monotone" dataKey="totalAmount" name="Total Amount (THB)" stroke="#10b981" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                            </ComposedChart>
                          </ResponsiveContainer>
                        </div>
                      </div>
                  ...
                  {Object.values(chartDataByLine).every(arr => arr.length === 0) && (
                    <div className="col-span-full flex flex-col items-center justify-center h-full py-10 text-slate-400">
                      {data.length === 0 ? 'ไม่มีข้อมูลสำหรับแสดงกราฟ' : 'ไม่พบข้อมูลในวันที่เลือก'}
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 border-b pb-2">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-slate-800">
                ข้อมูลที่บันทึกล่าสุด {filterDate && <span className="text-sm font-normal text-blue-600">(แสดงเฉพาะวันที่ {filterDate})</span>}
              </h2>
            </div>
            <button 
              onClick={exportCSV}
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right rounded-tr-lg">Total Amount</th>
                </tr>
              </thead>
              <tbody>
                {filteredData.map((row, index) => (
                  <tr key={row.id || index} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 font-medium">
                      <span className="bg-slate-100 text-slate-700 px-2 py-1 rounded text-xs">{row.line}</span>
                    </td>
                    <td className="px-4 py-3 font-medium text-indigo-600">{row.partCode}</td>
                    <td className="px-4 py-3">{row.partName}</td>
                    <td className="px-4 py-3 text-right font-medium">{row.quantity.toLocaleString('th-TH')}</td>
                    <td className="px-4 py-3 text-slate-500">{row.unit}</td>
                    <td className="px-4 py-3 text-right">฿ {row.amount.toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                ...
                {filteredData.length === 0 && (
                  <tr>
                    <td colSpan="8" className="px-4 py-8 text-center text-slate-400">
                      {data.length === 0 ? 'ยังไม่มีข้อมูล' : 'ไม่พบข้อมูลในวันที่เลือก'}
                    </td>
                  </tr>
                )}
              </tbody>
